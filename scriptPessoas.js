import dadosPessoas from './dados-pessoas.js';

let main = document.getElementsByTagName('main');
let itensCabecalho = ['Nome','Data - Entrada', 'Hora - Entrada', 'Data - Saída', 'Hora - Saída', 'Solicitante', 'Motivo', 'Placa', 'Materiais'];
let celulas = [];
let linhas = [];
...
function gerarTabela() {
    ...
    let celulasCorpoTabela = prepararCelulasPorLinha(dadosPessoas);
...
let prepararCelulasPorLinha = (dados) => {
  let celulasBody = [];
  dados.forEach((ficha) => {
    let celulasPorLinha = [ficha.nome, ficha.entrada.data, ficha.entrada.hora, ficha.saida.data, ficha.saida.hora, ficha.solicitante, ficha.motivo, ficha.placaVeiculo, (ficha.entrada.materiais != null || ficha.saida.materiais != null)  ? 'sim' : 'não'];
